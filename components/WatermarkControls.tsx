...
export function WatermarkControls(props: {
  ...
  const { mode,setMode,text,setText,image,setImage,onImageWatermark,imageError } = props;
  ...
  return <div className="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
    ...
    {mode==='text'?<><input value={text.text} onChange={(e)=>setText({...text,text:e.target.value})} placeholder='Watermark text' className='w-full rounded border border-slate-300 bg-transparent px-2 py-1 dark:border-slate-700'/><label className='block text-xs'>Font size <input type='range' min={12} max={180} value={text.fontSize} onChange={(e)=>setText({...text,fontSize:Number(e.target.value)})}/></label><label className='block text-xs'>Color <input type='color' value={text.color} onChange={(e)=>setText({...text,color:e.target.value})}/></label></>:<><input type='file' accept='.png,.webp,.svg,image/png,image/webp,image/svg+xml' onChange={(e)=>onImageWatermark(e.target.files?.[0])} className='text-xs'/>{imageError?<p className='text-xs text-red-500'>{imageError}</p>:null}<label className='block text-xs'>Size % <input type='range' min={5} max={60} value={image.sizePercent} onChange={(e)=>setImage({...image,sizePercent:Number(e.target.value)})}/></label></>}
