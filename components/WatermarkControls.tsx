...
export function WatermarkControls(props: {
  ...
  return <div className="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
    ...
    {mode==='text'?<><input value={text.text} onChange={(e)=>setText({...text,text:e.target.value})} placeholder='Watermark text' className='w-full rounded border border-slate-300 bg-transparent px-2 py-1 dark:border-slate-700'/><label className='block text-xs'>Watermark Size (%) <input type='range' min={1} max={12} step={0.1} value={text.sizePercent} onChange={(e)=>setText({...text,sizePercent:Number(e.target.value)})}/></label><label className='block text-xs'>Color <input type='color' value={text.color} onChange={(e)=>setText({...text,color:e.target.value || '#ffffff'})}/></label></>:<><input type='file' accept='.png,.jpg,.jpeg,.webp,.svg,image/png,image/jpeg,image/webp,image/svg+xml' onChange={(e)=>onImageWatermark(e.target.files?.[0])} className='text-xs'/>{imageError?<p className='text-xs text-red-500'>{imageError}</p>:null}{watermarkPreviewSrc?<div className='rounded border border-slate-300 p-2 dark:border-slate-700'><p className='mb-2 text-xs text-slate-500'>Watermark preview</p><img src={watermarkPreviewSrc} alt='Uploaded watermark preview' className='max-h-24 w-auto rounded' /></div>:<p className='text-xs text-amber-600 dark:text-amber-400'>Upload a watermark image first.</p>}<label className='block text-xs'>Watermark Size (%) <input type='range' min={5} max={60} value={image.sizePercent} onChange={(e)=>setImage({...image,sizePercent:Number(e.target.value)})}/></label></>}
    ...
    <label className='block text-xs'>Margin (%) <input type='range' min={0} max={10} step={0.1} value={base.marginPercent} onChange={(e)=>mode==='text'?setText({...text,marginPercent:Number(e.target.value)}):setImage({...image,marginPercent:Number(e.target.value)})}/></label>
