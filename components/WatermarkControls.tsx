...
export function WatermarkControls({ settings, setSettings }: { settings: WatermarkSettings; setSettings: (s: WatermarkSettings) => void }) {
  ...
  return <div className='space-y-2 rounded-lg border border-slate-200 p-4 dark:border-slate-800'>
    ...
    <div className='grid grid-cols-2 gap-2 text-xs'>{['repeat','lockAspectRatio','flipHorizontal','flipVertical','shadowEnabled','strokeEnabled'].map((k)=><label key={k}><input type='checkbox' checked={settings[k as keyof WatermarkSettings] as boolean} onChange={(e)=>update(k as keyof WatermarkSettings, e.target.checked as never)} /> {k}</label>)}</div>
